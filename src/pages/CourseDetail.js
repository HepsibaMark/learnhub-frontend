...
function CourseDetail() {
  ...
  useEffect(() => {
    axios.get(`http://127.0.0.1:5000/api/courses/${id}`)
      .then(res => {
  ...
  return (
    <div style={{ padding: '20px' }}>
      <button onClick={() => navigate('/')} style={{ padding: '8px 16px', background: '#2196F3', color: 'white', border: 'none', cursor: 'pointer', marginBottom: '20px' }}>
        ← Back to Courses
      </button>
